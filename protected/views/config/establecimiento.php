<?php
/* @var $this CrugeFieldvalueController */
/* @var $model CrugeFieldvalue */
/* @var $form CActiveForm */

$this->breadcrumbs=array(
	'Establecimiento por defecto'=>array('index'),
	'Actualizar',
);
?>

<?php
    $this->headers = ' Modificar establecimiento por defecto';
?>

<div class="form">

<?php $form=$this->beginWidget('CActiveForm', array(
	'id'=>'planes-form',
	'enableAjaxValidation'=>true,
)); ?>

	<?php echo $form->errorSummary($model); ?>

	<?php echo $form->label($model, 'Establecimiento'); ?>
	<?php echo $form->dropDownList($model,'value', Establecimientos::toList(), array('class'=>'form-control', 'style'=>'font-size:11px;')); ?>
	<?php echo $form->error($model,'value'); ?>

	<div class="clearfix"> <br>
		<p>		 
			<div class="col-lg-4 col-md-6 col-sm-7 col-xs-8 center">
				<button type="submit" class="btn btn-info btn-large"><i class="glyphicon glyphicon-ok icon-white"></i> Aplicar</button>
			</div>
		</p>
		<?php $this->endWidget(); ?>
	</div>
</div>	
