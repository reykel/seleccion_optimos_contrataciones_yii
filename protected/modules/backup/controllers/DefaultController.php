...
class DefaultController extends Controller
{
	...
	{
		$_name_str = $this->back_temp_file . date('Y.m.d_H.i.s');
		...
		foreach ($objetos_array as $key) {
			self::recorrer_archivos(self::lee_archivos("imagenes/".$key."/", "jpg"), $_name_str, $key, "salva");
		}

		$this->file_name =  $this->path . $_name_str . '.sql';
	...
	public function actionRestore($file = null)
	{
		...
		if ( isset($file))
		{
			...
			foreach ($objetos_array as $key) {
				if (file_exists("imagenes_backup/".$key."/". substr($file, 0, strlen($file)-4) ."/"))
					self::recorrer_archivos(self::lee_archivos("imagenes_backup/".$key."/". substr($file, 0, strlen($file)-4) ."/", "jpg"), substr($file, 0, strlen($file)-4), $key, "restaura");
			}
		...
		$this->render('site/index',array('error'=>$message));
	}
	...
	//recorrer_archivos(lee_archivos("imagenes/fotos/", "jpg"), $_folder_name, $_objeto);

	private function lee_archivos($directorio, $filtro)
	{
    	$archs=array();
	...
	private function recorrer_archivos($model, $_folder_name, $_objeto, $_operacion)
	{
		foreach ($model as $p) {
			if($_operacion == "salva")
				$imagedata = file_get_contents("imagenes/".$_objeto."/".$p);
			else
				$imagedata = file_get_contents("imagenes_backup/".$_objeto."/".$_folder_name."/".$p);

			$base64 = base64_encode($imagedata);			
			self::create_file($base64, $p, $_folder_name, $_objeto, $_operacion);
		}
	}

	private function create_file($Base64Img, $_file_name, $_folder_name, $_objeto, $_operacion)
	{
		if($_operacion == "salva")
			$carpeta = "imagenes_backup/".$_objeto."/".$_folder_name;
		else
			$carpeta = "imagenes/".$_objeto;
		...
		file_put_contents($carpeta."/".$_file_name, $Base64Img);    		
	}
}