...
class InstrumentosController extends Controller
{
	...
	public function actionCreate()
	{
		...
		{
			$model->attributes=$_POST['Instrumentos'];
			if($model->save())
	...
	{
		$model=new Instrumentos('search');
		$model->unsetAttributes();  // clear any default values

		if(isset($_GET['Instrumentos']))
			$model->attributes=$_GET['Instrumentos'];
	...
	{
		$model=Instrumentos::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param Instrumentos $model the model to be validated
	 */
	...
	public function actionReporte()
    {
		...
		if($_extension == 'pdf'){
			...
			$mPDF1->SetTitle("Reporte Instrumentos");
			$mPDF1->SetDisplayMode('fullpage');
			$mPDF1->WriteHTML($this->renderPartial('excel', '', true));
			$mPDF1->Output('Reporte_Instrumentos.pdf','I');
			exit;
		}else{
			$content = $this->renderPartial("excel", '',true);
			yii::app()->request->sendfile("Reporte_Instrumentos.".$_extension , $content);
		}
    }	
}
