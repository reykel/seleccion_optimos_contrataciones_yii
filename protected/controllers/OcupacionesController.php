...
class OcupacionesController extends Controller
{
	...
	public function actionCreate($_aspirante_id)
	{
		...
		{
			$model->attributes=$_POST['Ocupaciones'];
			if($model->save())
	...
	{
		$model=new Ocupaciones('search');
		$model->unsetAttributes();  // clear any default values

		if(isset($_GET['Ocupaciones']))
			$model->attributes=$_GET['Ocupaciones'];
	 ...
	 * @return Ocupaciones the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=Ocupaciones::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param Ocupaciones $model the model to be validated
	 */
	...
	public function actionReporte()
    {
		...
		if($_extension == 'pdf'){
			...
			$mPDF1->SetTitle("Reporte Ocupaciones");
			$mPDF1->SetDisplayMode('fullpage');
			$mPDF1->WriteHTML($this->renderPartial('excel', '', true));
			$mPDF1->Output('Reporte_Ocupaciones.pdf','I');
			exit;
		}else{
			$content = $this->renderPartial("excel", '',true);
			yii::app()->request->sendfile("Reporte_Ocupaciones.".$_extension , $content);
		}
    }	
}
