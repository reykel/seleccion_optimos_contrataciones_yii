...
class CertificacionesController extends Controller
{
	...
	public function actionCreate($_aspirante_id)
	{
		...
		{
			$model->attributes=$_POST['Certificaciones'];
			if($model->save())
	...
	{
		$model=new Certificaciones('search');
		$model->unsetAttributes();  // clear any default values

		if(isset($_GET['Certificaciones']))
			$model->attributes=$_GET['Certificaciones'];
	 ...
	 * @return Certificaciones the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=Certificaciones::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param Certificaciones $model the model to be validated
	 */
	...
	public function actionReporte()
    {
		...
		if($_extension == 'pdf'){
			...
			$mPDF1->SetTitle("Reporte Certificaciones");
			$mPDF1->SetDisplayMode('fullpage');
			$mPDF1->WriteHTML($this->renderPartial('excel', '', true));
			$mPDF1->Output('Reporte_Certificaciones.pdf','I');
			exit;
		}else{
			$content = $this->renderPartial("excel", '',true);
			yii::app()->request->sendfile("Reporte_Certificaciones.".$_extension , $content);
		}
    }	
}
