...
class Rank extends CActiveRecord
{
	...
	public function search()
	{
		...
		if(strlen($this->cargo_id) > 0){
			$criteria->compare('cargo_id', $this->cargo_id);
			$criteria->addInCondition('aspirante_id', AspiranteCargos::getArrayReturn($this->cargo_id));
		}else{
		...
		$criteria->limit = CrugeSystem::get_top_aspirantes();
	...
	public function get_aspirantes_ranking_list($_cargo_id)
	{
		$criteria=new CDbCriteria;

		if($_cargo_id != null)
			$criteria->addInCondition('id', AspiranteCargos::getArrayReturn($_cargo_id));

		$model_aspirantes = Aspirantes::model()->findAll($criteria);
		//$model_cargos = Cargos::model()->findAll();

		foreach ($model_aspirantes as $p) {
			self::valuate_aspirantes_ranking($p->id, $_cargo_id);
		}
	}
