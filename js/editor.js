		...
		init : function( options )
		{
			...
			var menuItems = { 'fonteffects': true,
						   ...
						   'insert_link': { "modal": true,
											...
											"icon":"fa fa-link", 
											"tooltip": "Insert Link Database", 
											"modalHeader": "Insert Hyperlink to database",
											...
											"onSave":function(){
												...
												if(targetURL ==''){
													methods.showMessage.apply(editorObj,["errMsg","Please enter database link"]);
													return false;
												}
												/*												
												...
												*/													
												...
												if(navigator.userAgent.match(/MSIE/i) || navigator.userAgent.match(/Windows NT.*Trident\//)){	
													var targetLink='<a href="'+targetURL+'" target="_DB_link">'+targetText+'</a>';
												...
												$(editorObj).data("editor").find('a[href="'+targetURL+'"]').each(function(){ $(this).attr("target", "_DB_link"); });
												$(".alert").alert("close");
						   ...
						   'insert_field': { "modal": true,
											...
											"onSave":function(){
												...
												if(navigator.userAgent.match(/MSIE/i) || navigator.userAgent.match(/Windows NT.*Trident\//)){	
													...
													alert('2 - ' + targetText);
													document.execCommand('insertHTML', false, '<table class="table" border="1" cellspacing="1" cellpadding="1" style="width: 400px;"><tbody><tr>' + targetText +'<td>&nbsp;</td><td>&nbsp;</td></tr><tr><td>&nbsp;</td><td>&nbsp;</td></tr></tbody></table>');
												}
												$(".alert").alert("close");
												$("#InsertField").modal("hide");
												$(editorObj).data("editor").focus();
												targetText = "";
												return false;
											}},											
